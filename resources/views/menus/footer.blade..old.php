                      ...
                      @foreach($menus as $menu)
                        <tr id = "{{ $menu->id }}" class="parent">
                            ...
                            <td class="text-right" >
                                ...
                                @endif
                                <a data-id="{{ $menu->id }}" href="#" class="edit btn btn-primary " data-toggle="modal" data-target="#editModal"><i class="fe-edit-2" ></i></a>
                                    <button type="button"  data-placement="top" data-id="{{$menu->id}}" title="" data-original-title="Delete" class="delete btn btn-danger ml-1 " data-toggle="modal" data-target="#deleteModal"><i class="fas fa-trash-alt"></i></button>        
                            </td>
                            @if(count(App\Menu::where('parent_id',$menu->id)->orderBy('position','asc')->get())>0)
                            	@foreach(App\Menu::where('parent_id',$menu->id)->orderBy('position','asc')->get() as $child)
                            		<tr id="{{ $child->id }}" data-id = "{{$menu->id}}">
                            			<td>~{{$child->name}}</td>
			                            <td>{{ $child->link }}</td>
                                ...
                                @endif
			                                 <a data-id="{{ $child->id }}" href="#" class="edit btn btn-primary " data-toggle="modal" data-target="#editModal2"><i class="fe-edit-2" ></i></a>
                                          <button type="button"  data-placement="top" data-id="{{$child->id}}" title="" data-original-title="Delete" class="delete btn btn-danger ml-1 " data-toggle="modal" data-target="#deleteChild"><i class="fas fa-trash-alt"></i></button>       
			                            </td>
                            	@endforeach
                            @endif
                        </tr>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Crete Footer</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('menuDetail.store') }}" method="post">
        	{{ csrf_field() }}
        ...
        <form action="{{ route('menuDetail.create') }}" method="get">
          ...
          <div class="form-group" id="selection">
            ...
            <select class="parent_id form-control" id="child" name="parent_id">
              @php
                $navs = App\Menu::where('type','footer')->where('parent_id',0)->get();
              @endphp
              @foreach($navs as $nav)
      ...
      if($(this).val() == 0){
		...
		$('.sort').sortable({
            ...
            change:function(event ,ui){
                var parent_id = ui.item.attr('data-id');
                console.log(parent_id);
            }
