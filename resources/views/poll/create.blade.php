                    ...
                    <div id="app">
                        ...
                        @verbatim
                            ...
                            <fieldset v-for="(question, idx) in questions" :key="idx">
                                <legend>Question #{{ idx + 1 }}<button v-if='questions.length > 1 && idx >= 1'
                                        class="mt-2 btn btn-primary mt-n1 ml-3" v-on:click="shift('up', idx)"><i
                                            class="fe-chevron-up"></i></button> <button v-if='questions.length > 1'
                                        class="mt-2 btn btn-danger mt-n1 ml-3" v-on:click="questions.splice(idx, 1)"><i
                                            class="fe-minus"></i></button> <button
                                        v-if='questions.length > 1 && idx < questions.length - 1'
                                        class="mt-2 btn btn-primary mt-n1 ml-3" v-on:click="shift('down', idx)"><i
                                            class="fe-chevron-down"></i></button>
                                </legend>
                                <div class="form-group">
                                ...
                                <div class="row form-group mb-3" v-for="(option, idx) in question.options" :key="idx">
                                    ...
                                    <div class="col-2 text-center">
                                        <button class="btn btn-success" v-if="question.options.length == idx + 1"
                                            v-on:click="question.options.push('')"><i class="fe-plus"></i></button>
                                        <button class="btn btn-danger" v-if="question.options.length > 1"
                                            v-on:click="question.options.splice(idx, 1)"><i class="fe-minus"></i></button>
                                    </div>
                                </div>
                            </fieldset>

                            <div class="form-group">
                                <button class="mt-2 btn btn-primary" v-on:click="questions.push({ text: '', options: [''] })"><i
                                        class="fe-plus"></i> Add Question</button>
                            </div>
