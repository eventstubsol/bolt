...
@section("breadcrumbs")
    <li class="breadcrumb-item active"><a href="{{ route("eventee.user.report",['id'=>$id]) }}">User Report</a></li>
@endsection
            ...
            $.post("{{ route('eventee.user.report.graph') }}",{'user_id':user_id,'start_date':start_date,'end_date':end_date,'event_id':"{{ $id }}"},function(response){
                ...
                if(response.code === 201){
                    ...
                    $.each(report,function(key,value){
                        $('#resultDiv').append('<table class="table  " id="childApp'+key+'"><thead><tr><th>'+ key + '</th></tr></thead><thead><tr><th>Type</th><th>Location</th><th>Entered At</th><th>Left At</th></tr></thead>');
