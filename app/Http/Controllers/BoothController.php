...
class BoothController extends Controller
{
  ...
  public function store(Request $request)
  {
    $request->validate([
      ...
      'room_id' => 'required',
      'userids' => 'required',
      'sessionRoom'=> 'required',
    ]);
    $booth = new Booth();
    $booth->name = $request->get("name");
    $booth->room_id = $request->get("room_id");
    $booth->type = $request->get("type");
    $booth->calendly_link=$request->get("calendly_link");
    if($request->has('event_id')){
      $booth->event_id = $request->event_id;
    }
    $booth->save();
