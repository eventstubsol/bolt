...
class AdminReportController extends Controller
{
    ...
    public function Dashboard(){
        ...
        $liveEvent = Event::where('end_date','>=',Carbon::now("UTC")->format('Y-m-d'))->count();
        $attendees = User::where('type','attendee')->where('online_status',1)->count();
        $recent = Event::whereBetween('start_date',[Carbon::now("UTC")->subDays(5)->format('Y-m-d'),Carbon::now("UTC")->format('Y-m-d')])->where('end_date','>=',Carbon::today())->orderBy('start_date','desc')->limit(5)->get();
        $last_activity = User::where('updated_at','<=',Carbon::now("UTC")->subDays(5)->format('Y-m-d H:i:s'))->where('type','eventee')->LIMIT(5)->get();
        ...
        $ending_event  =Event::whereBetween('end_date',[Carbon::now("UTC")->format('Y-m-d'),Carbon::now("UTC")->addDays(5)->format('Y-m-d')])->limit(5)->get();
    ...
    public function RecentEvent(){
        $recents = DB::table('events')
                ...
                ->whereBetween('events.start_date',[Carbon::now("UTC")->subDays(10)->format('Y-m-d'),Carbon::now("UTC")->format('Y-m-d')])
    ...
    public function LeastActiveUser(){
        $leastActive = User::where('updated_at','<=',Carbon::now("UTC")->subDays(5)->format('Y-m-d H:i:s'))->where('type','eventee')->get();
        return view('dashboard.admin.leastactive',compact('leastActive'));
    }

    public function RecentActiveUser(){
        $recentJoined = User::whereBetween('created_at',[Carbon::now("UTC")->subDays(5)->format('Y-m-d H:i:s'),Carbon::now("UTC")->format('Y-m-d H:i:s')])->where('type','eventee')->get();
    ...
    public function EventEnding(){
        $ending_event  =  DB::table('events')
                        ->join('users','events.user_id','=','users.id')
                        ->whereBetween('end_date',[Carbon::now("UTC")->format('Y-m-d'),Carbon::now("UTC")->addDays(5)->format('Y-m-d')])
