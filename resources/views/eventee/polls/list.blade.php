                        ...
                        <tr class="checkedbox" data-id="{{ $poll->id }}">
                            ...
                            <td class="text-right" >
                                <a href="{{ route("eventee.poll", [
                                        "poll" => $poll->id,'id'=>$id
                                    ]) }}" class="btn btn-primary" data-toggle="tooltip" title="Poll"><i class="fe-eye" ></i></a>
                             <button data-toggle="tooltip" data-placement="top" data-id="{{$poll->id}}" onclick="DeleteRoom(this)" data-toggle="tooltip" title="Delete" class="delete btn btn-danger ml-1 "  type="submit"><i class="fas fa-trash-alt"></i></button>        
                            </td>
                        </tr>
                      @endforeach
                    </tbody>
...
@section("scripts")
    ...
    <script>
        ...
        $(document).ready(function(){
            $("#buttons-container").append('<a class="btn btn-primary" href="{{ route("eventee.polls.create",["id"=>$id]) }}">Create New</a>');
        ...
        function BulkDelete(){
            if(deltype == 1){
                if(deleteArr.length < 1){
                    ...
                    $.post("{{ route('eventee.polls.bulkDelete')}}",{'ids': deleteArr},function(response){
               ...
               $.post("{{ route('eventee.polls.deleteAll') }}",{'id': "{{ $id }}" },function(response){
                    if(response.code == 200){
                            $('#successAlert').show()
                            $('#errorAlert').hide();
                            setTimeout(function(){ location.reload(); }, 2000);
