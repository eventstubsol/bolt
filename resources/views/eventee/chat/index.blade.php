                ...
                <form action="{{ route('settings.savechat',$id) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control"  name="enable_video_calling" required>
                            <option @if($settings->main->enable_video_calling) selected=true @endif  value=true>Enabled</option>
                            <option @if(!$settings->main->enable_video_calling) selected=true @endif value=false>Disabled</option>
                        </select>
                    </div>
                    <div class="form-group mb-3">
                        <label for="type">Voice Calling</label>
                        <select class="form-control"  name="enable_voice_calling" required>
                            <option @if($settings->main->enable_voice_calling) selected=true @endif  value=true>Enabled</option>
                            <option @if(!$settings->main->enable_voice_calling) selected=true @endif value=false>Disabled</option>
                        </select>
                    </div>
